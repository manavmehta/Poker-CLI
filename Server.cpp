...
void playingArea(int fd, sem_t* user_sem, int room_id)
{
  sendMessage(fd, "You have entered the Playing Area !!\n");
}
...
void waitingArea(int fd, string username, int room_id)
{
  ...
  {
    for (auto itr : roomSemaphores[room_id])
    {
      sem_post(itr);
    }
  }

  playingArea(fd, &temp_sem, room_id);
}
